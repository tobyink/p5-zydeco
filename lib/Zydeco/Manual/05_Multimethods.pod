...
There is no C<< multi factory >>, but you can use proxying to achieve
the same effect.
  ...
  package MyApp {
    ...
    class Person {
      ...
      multi method _new_person ( Str $name ) {
        $class->new( name => $name );
      }
      
      multi method _new_person ( Str $name, Num $age ) {
        $class->new( name => $name, age => $age );
      }
      
      multi method _new_person ( HashRef $args ) {
        $class->new( %$args );
      }
      
      factory new_person via _new_person;
    }
  }
...
We have looked at three ways to define methods: C<method>, C<factory>, and
C<< multi method >>. Now let's look at how to modify existing methods.
