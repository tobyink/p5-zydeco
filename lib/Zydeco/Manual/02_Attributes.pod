...
It's even possible to declare different method names for the getter and
setter methods:
...
Zydeco defaults to C<< is => rw >>, which is the most permissive, but you
should carefully consider how much you wish to directly expose your attribute
to code that uses your objects. In particular, making attributes read-only
...
Note that during object instatiation, there is no guaranteed order which
